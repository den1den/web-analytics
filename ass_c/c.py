...
from settings import nodes_header, out_nodes_filename, weight_commcenter, weight_normal, \
    classes, edges_header, out_edges_filename, all_years, starting_extra_id, output_format
...
for comm in classes:
    ...
    if output_format == 'normal':
        # try to group all the classes to the timestamps
        ...
        # Add a node for each year (So allot of nodes with only one timestamp
        for y in all_years:
            timestamp = year_array_to_gephi_timestamp([y, ])
            clazz = comm
            output.append((id, timestamp, clazz, ))
...
for global_id, years in years_map.items(): # get all the years this node was in
    ...
    if output_format == 'normal':
        ...
        for y in years:
            timestamp = year_array_to_gephi_timestamp([y, ])
            clazz = year_classification_mapping[y][global_id]
            output.append((id, timestamp, clazz, ))
...
for global_id, years in years_map.items():
    ...
    if output_format == 'normal':
        # collect all the communities for each year, and add edge for each community
        ...
        # Just add a edge for each year and each community
        for y in years:
            src = starting_id + comm # from the hidden comunity node
            id = starting_extra_id + len(output)
            timestamp = year_array_to_gephi_timestamp([y, ])
